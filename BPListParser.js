...
const extract = async function(path, destination) {
  // read file
  let obj = await bplist.parseFile(path);
  objmain = obj[0]["$objects"][1]["NS.data"] || obj[0]["$objects"][1];

  // fetch data from file
  let actualObj = await bplist.parseFile(objmain);
  ...
  keys.forEach(function(uid) {
    ...
    // nested dictionary with references
    try {
      if (v["NS.keys"] && v["NS.objects"]) {
        let dict = {};
        let i = 0;
        v["NS.keys"].forEach(function(key) {
          key = actualObj[0]["$objects"][key.UID];
          dict[key] = actualObj[0]["$objects"][v["NS.objects"][i].UID];
          i++;
        })
        v = dict;
      }
  ...
  fs.writeFile(destination, result, function() {
    console.log("Written result to: " + destination);
  })

}
}).call(this)}).call(this,{"isBuffer":require("./node_modules/is-buffer/index.js")})
},{"./node_modules/is-buffer/index.js":8,"bplist-parser":4}],2:[function(require,module,exports){
'use strict'
...
},{"big-integer":3,"buffer":6,"fs":5}],5:[function(require,module,exports){
