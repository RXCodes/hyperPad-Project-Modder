...
BPList.maxObjectSize = 100 * 1000 * 1000; // 100Meg
BPList.maxObjectCount = 32768;
...
BPList.UID = function(id) {
  this.UID = id;
};

BPList.parseFile = function (fileNameOrBuffer, callback) {
...
BPList.parseFileSync = function (fileNameOrBuffer) {
  if (!Buffer.isBuffer(fileNameOrBuffer)) {
...
BPList.parseBuffer = function (buffer) {
  // check header
  ...
  if (numObjects > BPList.maxObjectCount) {
  ...
  function parseObject(tableOffset) {
    ...
    function parseInteger() {
      ...
      if (length < BPList.maxObjectSize) {
        ...
        throw new Error("Too little heap space available! Wanted to read " + length + " bytes, but only " + BPList.maxObjectSize + " are available.");
    ...
    function parseArray() {
      ...
      if (length * objectRefSize > BPList.maxObjectSize) {
    ...
    function parseDictionary() {
      ...
      if (length * 2 * objectRefSize > BPList.maxObjectSize) {
        throw new Error("Too little heap space available!");
      }
      if (debug) {
